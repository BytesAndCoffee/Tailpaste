...
class HealthMonitor:
    ...
    def record_health_result(
        self, session_id: str, status: str, details: Dict[str, Any]
    ) -> bool:
    ...
    def should_trigger_recovery(
        self, failure_pattern: Dict[str, Any]
    ) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Health monitoring utility for CI/CD integration"
    )
    parser.add_argument(
        "--results-dir", default="/tmp", help="Directory to store health results"
    )
    ...
    record_parser.add_argument(
        "--session-id", required=True, help="Health check session ID"
    )
    record_parser.add_argument(
        "--status",
        required=True,
    ...
    latest_parser = subparsers.add_parser(
        "latest", help="Get latest health check result"
    )
    ...
    recovery_parser = subparsers.add_parser(
        "recovery", help="Check if recovery should be triggered"
    )
    ...
    cleanup_parser.add_argument(
        "--max-age-hours", type=int, default=24, help="Maximum age of files to keep"
    )
    cleanup_parser.add_argument(
        "--max-files", type=int, default=50, help="Maximum number of files to keep"
    )
    ...
    if args.command == "record":
        ...
        success = monitor.cleanup_old_results(
            max_age_hours=args.max_age_hours, max_files=args.max_files
        )
        sys.exit(0 if success else 1)
