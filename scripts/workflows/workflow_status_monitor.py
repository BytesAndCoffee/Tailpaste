...
class WorkflowStatusMonitor:
    ...
    def get_workflow_runs(self, workflow: str, limit: int = 20) -> List[Dict]:
        ...
        workflow_file = self.workflow_graph.get(workflow, {}).get('file', f'{workflow}.yml')
        
        success, output = self._run_gh_command([
            'run', 'list', 
            '--workflow', workflow_file,
            '--limit', str(limit),
            '--json', 'databaseId,status,conclusion,createdAt,updatedAt,headSha,event,displayTitle'
        ])
    ...
    def _calculate_workflow_health(self, runs: List[Dict]) -> str:
        ...
        recent_runs = runs[:5]
        conclusions = [run.get('conclusion') for run in recent_runs if run.get('conclusion')]
        ...
        if latest_run.get('status') == 'in_progress':
            ...
            if created_at:
                try:
                    ...
                    duration = (datetime.now(timezone.utc) - created_time).total_seconds() / 60
                    max_duration = workflow_config.get('max_duration_minutes', 30)
                    
                    if duration > max_duration:
                        issues.append(f"Run has been in progress for {duration:.1f} minutes (max: {max_duration})")
                except:
    ...
    def _calculate_workflow_metrics(self, workflow: str, runs: List[Dict]) -> Dict[str, Any]:
        ...
        success_rate = conclusions.count('success') / len(conclusions) if conclusions else 0
    ...
    def check_system_state_consistency(self) -> Dict[str, Any]:
        ...
        consistency_checks = {
            ...
            'orchestration_state': self._check_orchestration_state_consistency(variables)
        }
        
        return consistency_checks

    def _check_deployment_state_consistency(self, variables: Dict[str, str]) -> Dict[str, Any]:
        ...
        if deployment_in_progress == 'true' and deployment_started_at:
            try:
                ...
                duration = (datetime.now(timezone.utc) - started_time).total_seconds() / 60
    ...
    def _check_circuit_breaker_consistency(self, variables: Dict[str, str]) -> Dict[str, Any]:
        ...
        should_be_open = (recovery_failures >= recovery_threshold or 
                         deployment_failures >= deployment_threshold)
        
        if should_be_open and cb_status != 'open':
            issues.append(f'Circuit breaker should be open (R:{recovery_failures}/{recovery_threshold}, D:{deployment_failures}/{deployment_threshold})')
    ...
    def _check_recovery_state_consistency(self, variables: Dict[str, str]) -> Dict[str, Any]:
        ...
        return {
            ...
            'recovery_session_id': recovery_session[:16] + '...' if recovery_session else 'none'
        }

    def _check_orchestration_state_consistency(self, variables: Dict[str, str]) -> Dict[str, Any]:
        ...
        if orchestration_session and orchestration_start:
            try:
                start_time = datetime.fromisoformat(orchestration_start.replace('Z', '+00:00'))
                duration = (datetime.now(timezone.utc) - start_time).total_seconds() / 60
                if duration > 15:  # 15 minutes
                    issues.append(f'Orchestration session stuck for {duration:.1f} minutes')
            except:
    ...
    def _analyze_overall_health(self, report: Dict[str, Any]) -> None:
        ...
        if unhealthy_workflows:
            recommendations.append(f'Investigate unhealthy workflows: {", ".join(unhealthy_workflows)}')
        
        if degraded_workflows:
            recommendations.append(f'Monitor degraded workflows: {", ".join(degraded_workflows)}')
    ...
    def print_summary(self, report: Dict[str, Any]) -> None:
        ...
        for workflow, status in report['workflow_statuses'].items():
            ...
            print(f"  {health_emoji} {workflow}: {health.upper()} (last: {workflow_status})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Monitor CI/CD workflow status and health")
    ...
    parser.add_argument('--check-dependencies', action='store_true', help='Check workflow dependencies')
    parser.add_argument('--check-sequencing', action='store_true', help='Check workflow sequencing')
    parser.add_argument('--check-consistency', action='store_true', help='Check system state consistency')
    ...
    try:
        ...
        if args.workflow:
            ...
            for check_name, result in consistency.items():
                ...
                print(f"{check_name}: {'✅ Consistent' if status == 'consistent' else '❌ Inconsistent'}")
