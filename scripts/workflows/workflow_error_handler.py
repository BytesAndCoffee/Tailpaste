...
class WorkflowErrorHandler:
    ...
    def get_failed_workflow_runs(self, hours: int = 24) -> List[Dict]:
        ...
        success, output = self._run_gh_command(
            [
                ...
                "100",
                "--json",
                "databaseId,status,conclusion,createdAt,workflowName,displayTitle,headSha",
            ]
        )
    ...
    def get_workflow_run_logs(self, run_id: str) -> Dict[str, Any]:
        ...
        success, output = self._run_gh_command(["run", "view", run_id, "--log-failed"])
    ...
    def analyze_error_patterns(self, failed_runs: List[Dict]) -> Dict[str, Any]:
        ...
        for run in failed_runs:
            ...
            workflow_failures[workflow_name] = (
                workflow_failures.get(workflow_name, 0) + 1
            )
        ...
        for category in error_categories:
            ...
            error_categories[category]["avg_confidence"] = (
                error_categories[category]["confidence_sum"] / count
            )
        ...
        if hourly_distribution:
            ...
            if peak_hour[1] > total_runs * 0.3:  # More than 30% of failures in one hour
                patterns.append(
                    f"Failure spike at hour {peak_hour[0]}: {peak_hour[1]} failures"
                )
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": category,
                            "automated": strategy
                            in ["retry", "resource_cleanup", "wait_and_retry"],
                        }
                    )
    ...
    def execute_automated_recovery(
        ...
        try:
            if action == "retry":
                ...
                result["success"] = True
                result["details"] = (
                    "Scheduled delayed retry (not implemented in this demo)"
                )
                ...
                success, output = self._run_gh_command(
                    [
                        ...
                        self.repository,
                        "--field",
                        "orchestration_action=emergency-coordination",
                        "--field",
                ...
                result["details"] = (
                    f"Automated execution not implemented for action: {action}"
                )
    ...
    def _clear_stuck_processes(self) -> bool:
        ...
        try:
            ...
            self._run_gh_command(
                ["variable", "delete", "RECOVERY_SESSION_ID", "--repo", self.repository]
            )
    ...
    def generate_error_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                "total_failures": error_analysis.get("total_failures", 0),
                "unique_workflows_affected": len(
                    error_analysis.get("workflow_failure_rates", {})
                ),
                "error_categories_detected": len(
                    error_analysis.get("error_categories", {})
                ),
                "automated_actions_available": len(
                    [r for r in recommendations if r.get("automated", False)]
                ),
                "manual_actions_required": len(
                    [r for r in recommendations if not r.get("automated", False)]
                ),
            },
        }
    ...
    def print_error_summary(self, report: Dict[str, Any]) -> None:
        ...
        print(
            f"  Automated Actions Available: {summary.get('automated_actions_available', 0)}"
        )
        ...
        if workflow_failures:
            ...
            for workflow, count in sorted(
                workflow_failures.items(), key=lambda x: x[1], reverse=True
            ):
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations[:10], 1):  # Show first 10
                ...
                automated = (
                    "🤖 Automated" if rec.get("automated", False) else "👤 Manual"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Handle and analyze CI/CD workflow errors"
    )
    ...
    parser.add_argument(
        "--execute-automated",
        action="store_true",
        help="Execute automated recovery actions",
    )
    ...
    try:
        ...
        if args.run_id:
            ...
            if args.execute_automated:
                ...
                if automated_recs:
                    ...
                    for rec in automated_recs:
                        ...
                        if result["success"]:
                            print(
                                f"  ✅ Success: {result.get('details', 'No details')}"
                            )
                        else:
            ...
            critical_recs = len(
                [
                    r
                    for r in report.get("recovery_recommendations", [])
