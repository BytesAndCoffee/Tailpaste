...
class CircuitBreakerManager:
    ...
    def get_variable(self, name: str) -> Optional[str]:
        ...
        success, output = self._run_gh_command(['variable', 'get', name, '--repo', self.repo])
        return output if success else None

    def set_variable(self, name: str, value: str) -> bool:
        """Set a GitHub repository variable"""
        success, _ = self._run_gh_command(['variable', 'set', name, '--body', value, '--repo', self.repo])
        return success
    ...
    def get_circuit_breaker_status(self) -> Dict[str, str]:
        ...
        status = {
            ...
            'recovery_failure_count': self.get_variable('RECOVERY_FAILURE_COUNT') or '0',
            'deployment_failure_count': self.get_variable('DEPLOYMENT_FAILURE_COUNT') or '0',
            'recovery_threshold': self.get_variable('CIRCUIT_BREAKER_THRESHOLD') or '3',
            'deployment_threshold': self.get_variable('DEPLOYMENT_CIRCUIT_BREAKER_THRESHOLD') or '5',
            'last_recovery_failure_time': self.get_variable('LAST_RECOVERY_FAILURE_TIME') or '',
            'last_recovery_failure_reason': self.get_variable('LAST_RECOVERY_FAILURE_REASON') or '',
            'last_deployment_failure_time': self.get_variable('LAST_DEPLOYMENT_FAILURE_TIME') or '',
            'last_deployment_failure_reason': self.get_variable('LAST_DEPLOYMENT_FAILURE_REASON') or '',
            ...
            'override_reason': self.get_variable('CIRCUIT_BREAKER_OVERRIDE_REASON') or ''
        }
        return status
    ...
    def print_status(self) -> None:
        ...
        print(f"Recovery Failures: {status['recovery_failure_count']} / {status['recovery_threshold']}")
        print(f"Deployment Failures: {status['deployment_failure_count']} / {status['deployment_threshold']}")
    ...
    def get_recovery_history(self) -> Dict:
        ...
        history = {
            ...
            'last_recovery_reason': self.get_variable('LAST_RECOVERY_TRIGGER_REASON') or '',
            'last_completed_recovery': self.get_variable('LAST_COMPLETED_RECOVERY') or '',
            'last_recovery_session': self.get_variable('LAST_COMPLETED_RECOVERY_SESSION') or '',
            'recovery_successful': self.get_variable('RECOVERY_SUCCESSFUL') or '',
            'last_redeployment_trigger': self.get_variable('LAST_REDEPLOYMENT_TRIGGER') or '',
            'last_redeployment_artifact': self.get_variable('LAST_REDEPLOYMENT_ARTIFACT') or '',
        }
        return history
    ...
    def print_recovery_history(self) -> None:
        ...
        if history['last_completed_recovery']:
            ...
            if history['recovery_successful']:
                success_status = "✅ Success" if history['recovery_successful'] == 'true' else "❌ Failed"
    ...
    def _log_circuit_breaker_event(self, action: str, reason: str, triggered_by: str) -> None:
    ...
    def print_event_log(self) -> None:
        ...
        if not events:
            ...
            for event in events[-10:]:  # Show last 10 events
                ...
                action_emoji = "🚫" if action == "opened" else "✅" if action == "closed" else "🔄"
    ...
    def check_thresholds(self) -> Dict[str, bool]:
        ...
        recovery_exceeded = int(status['recovery_failure_count']) >= int(status['recovery_threshold'])
        deployment_exceeded = int(status['deployment_failure_count']) >= int(status['deployment_threshold'])
    ...
    def auto_open_if_needed(self) -> bool:
        ...
        if thresholds['any_threshold_exceeded'] and self.get_variable('CIRCUIT_BREAKER_STATUS') != 'open':
            reasons = []
    ...
    def set_deployment_threshold(self, threshold: int) -> bool:
        ...
        success = self.set_variable('DEPLOYMENT_CIRCUIT_BREAKER_THRESHOLD', str(threshold))
...
def main():
    ...
    close_parser.add_argument('--keep-failures', action='store_true', help='Keep failure counts')
    
    # Threshold commands
    threshold_parser = subparsers.add_parser('threshold', help='Set recovery failure threshold')
    threshold_parser.add_argument('value', type=int, help='Threshold value')
    
    deployment_threshold_parser = subparsers.add_parser('deployment-threshold', help='Set deployment failure threshold')
    ...
    # Event log command
    events_parser = subparsers.add_parser('events', help='Show circuit breaker event log')
    
    # Check command
    check_parser = subparsers.add_parser('check', help='Check thresholds and auto-open if needed')
    ...
    try:
        ...
        if args.command == 'status':
            ...
            print(f"Recovery threshold exceeded: {'✅ Yes' if thresholds['recovery_threshold_exceeded'] else '❌ No'}")
            print(f"Deployment threshold exceeded: {'✅ Yes' if thresholds['deployment_threshold_exceeded'] else '❌ No'}")
            
            if manager.auto_open_if_needed():
                print("🚫 Circuit breaker automatically opened due to threshold violations")
                return 1
            else:
