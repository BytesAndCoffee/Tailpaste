...
class ManualActionManager:
    ...
    def load_manual_actions_history(self) -> Dict:
        ...
        if self.manual_actions_file.exists():
            try:
                ...
                print(f"Warning: Could not load manual actions history file: {e}", file=sys.stderr)
    ...
    def save_manual_actions_history(self, data: Dict) -> None:
        ...
        try:
            ...
            print(f"Error: Could not save manual actions history file: {e}", file=sys.stderr)
            sys.exit(1)
    ...
    def validate_manual_action_parameters(
        ...
        has_sensitive_bypass = any(bypass_flags.get(flag, False) for flag in sensitive_operations)
        
        if has_sensitive_bypass and len(reason.strip()) < 20:
            return False, "Sensitive operations require detailed reasons (minimum 20 characters)"
    ...
    def record_manual_action(
        ...
        action_id = f"manual-{action_type}-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}"
    ...
    def generate_manual_actions_report(
        ...
        recent_actions = [
            ...
            if datetime.fromisoformat(action.get("timestamp", "").replace("Z", "+00:00")) > cutoff_date
        ]
    ...
    def cleanup_old_manual_actions(self, days_to_keep: int = 180) -> int:
        ...
        data["manual_actions"] = [
            action for action in data["manual_actions"]
            if datetime.fromisoformat(action.get("timestamp", "").replace("Z", "+00:00")) > cutoff_date
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Manage manual action logging and auditing")
    ...
    validate_parser.add_argument(
        "--override-circuit-breaker", action="store_true", help="Override circuit breaker flag"
    )
    validate_parser.add_argument(
    ...
    record_parser = subparsers.add_parser(
        "record-action", help="Record manual action"
    )
    ...
    record_parser.add_argument(
        "--workflow-run-id", help="Workflow run ID"
    )
    record_parser.add_argument(
        "--artifact-digest", help="Artifact digest"
    )
    record_parser.add_argument(
        "--bypass-gating", action="store_true", help="Bypass gating flag"
    )
    ...
    update_parser.add_argument(
        "--action-id", required=True, help="Manual action ID"
    )
    update_parser.add_argument(
    ...
    status_parser = subparsers.add_parser(
        "get-status", help="Get manual action status"
    )
    ...
    list_parser.add_argument(
        "--repository", help="Repository name (owner/repo)"
    )
    list_parser.add_argument(
        "--action-type", help="Filter by action type"
    )
    list_parser.add_argument(
    ...
    try:
        if args.command == "validate-parameters":
            ...
            action_id = manager.record_manual_action(
                args.action_type, args.actor, args.reason, args.repository,
                args.workflow_run_id, args.artifact_digest, bypass_flags
            )
            print(action_id)
            ...
            if actions:
                for action in actions:
                    ...
                    bypass_flags = action.get("bypass_flags", {})
                    active_bypasses = [flag for flag, value in bypass_flags.items() if value]
                    bypass_info = f" [{', '.join(active_bypasses)}]" if active_bypasses else ""
                    
                    print(f"{status_emoji} {action['id']} - {action.get('action_type', 'unknown')} - {action.get('actor', 'unknown')} - {action.get('timestamp', 'N/A')}{bypass_info}")
            else:
