...
class RollbackManager:
    ...
    def load_rollback_history(self) -> Dict:
        ...
        if self.rollback_history_file.exists():
            try:
                ...
                print(f"Warning: Could not load rollback history file: {e}", file=sys.stderr)
    ...
    def validate_rollback_prerequisites(self, repository: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(
                ["gh", "variable", "get", "BACKUP_ARTIFACT_DIGEST", "--repo", repository],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode != 0:
                return False, "No backup artifact digest found - cannot perform digest-based rollback"
            ...
            result = subprocess.run(
                ["gh", "variable", "get", "DEPLOYED_ARTIFACT_DIGEST", "--repo", repository],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if current_digest == backup_digest:
                return False, "Current deployment already matches backup artifact - no rollback needed"
            
            return True, f"Rollback prerequisites validated - can rollback to {backup_digest}"
        ...
        for rollback in data["rollbacks"]:
            if rollback["id"] == rollback_id:
                if stage in rollback["stages"]:
                    ...
                    print(f"Warning: Unknown stage {stage} for rollback {rollback_id}", file=sys.stderr)
                    return
    ...
    def get_recent_rollbacks(self, repository: str, limit: int = 10) -> List[Dict]:
        ...
        repo_rollbacks = [
            rb for rb in data["rollbacks"] 
            if rb.get("repository") == repository
        ]
    ...
    def validate_rollback_target(
        ...
        try:
            ...
            cmd = ["docker", "manifest", "inspect", f"{registry}/{repository}@{target_digest}"]
    ...
    def generate_rollback_report(self, repository: str) -> str:
        ...
        # Summary statistics
        successful_count = len([rb for rb in recent_rollbacks if rb.get("status") == "successful"])
        failed_count = len([rb for rb in recent_rollbacks if rb.get("status") == "failed"])
        pending_count = len([rb for rb in recent_rollbacks if rb.get("status") not in ["successful", "failed"]])
    ...
    def cleanup_old_rollback_records(self, days_to_keep: int = 90) -> int:
        ...
        data["rollbacks"] = [
            rb for rb in data["rollbacks"]
            if datetime.fromisoformat(rb.get("initiated_at", "").replace("Z", "+00:00")) > cutoff_date
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Manage rollback operations and tracking")
    ...
    record_parser.add_argument(
        "--method", required=True, choices=["digest-based", "file-based"], 
    ...
    record_parser.add_argument(
        "--reason", help="Reason for rollback"
    )
    ...
    update_parser.add_argument(
        "--rollback-id", required=True, help="Rollback ID"
    )
    update_parser.add_argument(
        "--stage", required=True, 
    ...
    update_parser.add_argument(
        "--details", help="Additional details"
    )

    # Get status command
    status_parser = subparsers.add_parser(
        "get-status", help="Get rollback status"
    )
    ...
    list_parser = subparsers.add_parser(
        "list-recent", help="List recent rollbacks"
    )
    ...
    try:
        if args.command == "validate-prerequisites":
            ...
            rollback_id = manager.record_rollback_attempt(
                args.repository, args.target_digest, args.method, 
                args.initiated_by, args.reason
            )
            print(rollback_id)
            ...
            if rollbacks:
                for rb in rollbacks:
                    ...
                    print(f"{status_emoji} {rb['id']} - {rb.get('status', 'unknown')} - {rb.get('initiated_at', 'N/A')}")
            else:
