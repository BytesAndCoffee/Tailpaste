...
def get_critical_recommendations():
    ...
    try:
        ...
        critical_recs = [r for r in report.get('recommendations', []) if 'CRITICAL:' in r]
        ...
        if 'error' in consistency:
            ...
            for check_name, check_result in consistency.items():
                if isinstance(check_result, dict):
                    ...
                    print(f'- **{check_name.replace("_", " ").title()}**: {status_emoji} {status.upper()}')
...
def sync_state():
    ...
    try:
        ...
        def set_variable(name, value):
            cmd = ['gh', 'variable', 'set', name, '--body', value, '--repo', os.getenv('GITHUB_REPOSITORY')]
        ...
        critical_count = len([r for r in report.get('recommendations', []) if 'CRITICAL:' in r])
